...
public:
	...
	{
		this->identifier = key; //parameters
		this->scope = value;
		this->type = type;
		this->lineNo = lineNo;
		next = NULL;
...
bool SymbolTable::modify(string id, string s,    // Function to modify an identifier
						string t, int l)
{
	int index = hashf(id);
...
bool SymbolTable::deleteRecord(string id)   // Function to delete an identifier
{
	int index = hashf(id);
	Node* tmp = head[index];
	Node* par = head[index];
...
string SymbolTable::find(string id)    // Function to find an identifier
{
	int index = hashf(id);
	Node* start = head[index];
...
bool SymbolTable::insert(string id, string scope,    // Function to insert an identifier
						string Type, int lineno)
{
	int index = hashf(id);
...
int main()
{
	...
	if (st.insert("operator", "global", "addition", 2))
		cout << " -successfully\n\n";
	else
		cout << "\nFailed to insert\n";
	...
	// Delete number
	if (st.deleteRecord("number"))
		cout << "number is deleted\n";
	else
		cout << "\nFailed to delete\n";

	// modify 'operator'
	if (st.modify("operatot", "global", "addition", 3))
		cout << "\nNumber Identifier updated\n";

	// find and print 'operator
	check = st.find("operator");
	if (check != "-1")
		cout << "Identifier Is present\n";
	else
		cout << "\nIdentifier Not Present";
